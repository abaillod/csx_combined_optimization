...
with MPIFiniteDifference(prob.objective, mpi, diff_method=diff_method, abs_step=finite_difference_abs_step, rel_step=finite_difference_rel_step) as prob_jacobian:
    ...
    info = ({'Nfeval':0})
    f = lambda x: fun(x, prob_jacobian=prob_jacobian, info=info)
    dofs = np.concatenate((JF.x, vmec.x))
    np.random.seed(1)
    ...
    for eps in myeps:
        J1, _ = f(dofs + eps*h)
        J2, _ = f(dofs - eps*h)
        print(f"Taylor test, eps={eps}, J'-FD(J)={(J1-J2)/(2*eps) - dJh}")
        err.append((J1-J2)/(2*eps) - dJh)

    plt.figure()
    plt.loglog(myeps, np.abs(err), '*')
    plt.xlabel(r'$\epsilon$')
    plt.ylabel(r"$|J'(x)-FD(J(x)|$")
    plt.show()
