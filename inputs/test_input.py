...
inputs['CC_THRESHOLD'] = 0.1
inputs['CC_WEIGHT'] = Weight(1)
inputs['directory'] = 'runs/test'

# SURFACE
inputs['vmec'] = dict()
inputs['vmec']['filename'] = 'inputs/vmec_inputs/input.vacuum_low_res' 
inputs['vmec']['verbose'] = False
inputs['vmec']['nphi'] = 12
inputs['vmec']['ntheta'] = 10

inputs['vmec']['dofs'] = dict()
inputs['vmec']['dofs']['internal_mpol'] = 1
inputs['vmec']['dofs']['internal_ntor'] = 1
...
inputs['vmec']['target']['qa_surface'] = np.linspace(0,1,3,endpoint=True) # Weight for QA is 1.
inputs['vmec']['target']['qa_ntheta'] = 16
inputs['vmec']['target']['qa_nphi'] = 15
...
inputs['cnt_coils']['geometry']['filename'] = 'inputs/coil_inputs/flux_100_bs_cssc_cssc.json'

inputs['cnt_coils']['dofs'] = dict()
inputs['cnt_coils']['dofs']['IL_order'] = 1
...
inputs['wp_coils']['target']['msc_weight'] = Weight(1)
inputs['wp_coils']['target']['maxc_threshold'] = 50
inputs['wp_coils']['target']['maxc_weight'] = Weight(1)
...
inputs['numerics']['MAXITER_stage_2'] = 2 # NUmber of iteration for initial stage two optimization
inputs['numerics']['MAXITER_single_stage'] = 2 # NUmber of iteration for combined optimization
